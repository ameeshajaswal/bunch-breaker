...
def normalize_intersection_name(name):
    """Normalize intersection names for matching"""
    if not name:
        return name
    name = name.upper()
...
def load_equity_data():
    """Load income quartile data for equity weighting"""
    equity_dict = {}
    ...
    # CRITICAL: Hardcode Jane-Finch for demo (matches your database exactly)
...
def get_equity_factor(intersection, equity_data):
    """Get equity factor for an intersection with fallback"""
    if not equity_data:
        return 1.0
    ...
    # Check for Jane-Finch pattern (matches your database)
    ...
    # Check original name (not normalized)
...
async def debug_hotspots(
    ...
    """Debug endpoint to see what hotspots are in database"""
